...
+ (id)trieWithCEDICTFile:(NSString *)aPath { return [[[self alloc] initWithCEDICTFile:aPath] autorelease]; }
...
- (id)initWithCEDICTFile:(NSString*)aPath
{
    if( (self = [self init]) != nil )
	{
        NSLog(@"start %f",[NSDate timeIntervalSinceReferenceDate]);
        
        FileReader* fileReader = [[[FileReader alloc] initWithFilePath:aPath]autorelease];
        if (!fileReader)
            @throw [NSException exceptionWithName:NSInvalidArgumentException reason:[NSString stringWithFormat:@"CEDICT file does not exist in current directory %@",aPath] userInfo:nil];
        ...
        while ((line = [fileReader readLine]))
        {
            ...
            NSString *aKey = array2[1];
            NSString *aObject = [array1[0] componentsSeparatedByCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"[]"]][1];
    
            _count += setObjectForKey( [self root],aObject, aKey, keyComponentForString );
     ...
     NSLog(@"end %f",[NSDate timeIntervalSinceReferenceDate]);
